...
/**
 * The most difficult part is to parse the torrentinfo.
 * Some torrents have 1 column and some have 2.
 * This code should be checked if it can run faster.
 */
public class Net {
	Log logger = null;
	...
	public TorrentInfo fetchInfo(int id) throws InterruptedException {
		...
		for (;;) {
			...
			if (e != null) {
				info.error = e.getFirstElement("h2").getTextExtractor().toString();
				return info;
			}
			...
			while (cols.hasNext()) {
				...
				for (int i=0; i < dts.size(); i++) {
					...
					if (dt.equals("Type:")) {
						//Get cat
						String cat = dds.get(i).getFirstElement("a").getAttributeValue("href");
						...
						//Get rating
						String rating = dds.get(i).getTextExtractor().toString();
						...
						//Get date
						String dateString = dds.get(i).getTextExtractor().toString();
						...
						try {
							...
							E.printStackTrace();
							log("Could not parse date for id "+id+". Date string is \""+dateString+"\".",true);
						}
			...
			//Get desc
			String desc = details.getFirstElementByClass("nfo").getFirstElement("pre").getContent().toString();
			...
			/*catch (Exception E) {
				BufferedWriter out = new BufferedWriter(new FileWriter("source-"+id+".txt"));
				out.write(source.toString());
				out.close();
			}*/
			//System.out.println("Desc: "+desc);
			/*
			BufferedWriter out = new BufferedWriter(new FileWriter("desc-"+id+".txt"));
			out.write(desc);
			out.close();
			*/
	...
	public Torrent fetchTorrent(int id) throws InterruptedException {
		...
		for (;;) {
			try {
				...
				//Make sure we got everything
				if (pos != size) {
					log("Id "+id+": Did not read whole torrent. Read "+pos+" out of "+size+" bytes!",true);
					throw new RuntimeException();
				}
			} catch (IOException e) {
				e.printStackTrace();
				log("IOException when fetching torrent for id "+id+", retrying.",true);
				Thread.sleep(2000);
				continue;
			}
			break;
	...
	public static void main(String[] args) throws SQLException, InterruptedException {
		...
		Net net = new Net();
/*		DB db = new DB();
		db.open("~/Public/Drop Box/tpb/tpb.db");
		db.init();
		*/
		//Fetch
	...
	/*	//Put in db ...
		System.out.println("Putting stuff in db.");
		db.begin();
		db.insertInfo(info);
		db.insertTorrent(torrent);
		db.commit();
		*/
		//... and get it back again (doesn't work with this driver - too many rows I guess)
		/*System.out.println();
		info = null;
		torrent = null;
		System.out.println("Getting stuff from db.");
		info = db.getInfo(id);
		torrent = db.getTorrent(id);
